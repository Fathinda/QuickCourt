  ...
  Future<void> _fetchBookingDetail() async {
    try {
      final response = await http.get(
        Uri.parse("http://192.168.1.10:8000/api/bookings/${widget.bookingId}"),
        headers: {
                    ...
                    : () async {
                        ...
                        try {
                          final request = http.MultipartRequest(
                            'POST',
                            Uri.parse(
                                "http://192.168.1.10:8000/api/payments/$paymentId/pay"),
                          );
  ...
  Future<void> _payBooking() async {
    ...
    try {
      final response = await http.post(
        Uri.parse(
            "http://192.168.1.10:8000/api/bookings/${widget.bookingId}/pay"),
        headers: {
          "Accept": "application/json",
          if (token != null) "Authorization": "Bearer $token",
        },
  ...
  Future<void> _uploadReceipt(int paymentId) async {
    ...
    try {
      final request = http.MultipartRequest(
        'POST',
        Uri.parse(
            "http://192.168.1.10:8000/api/payments/$paymentId/upload-receipt"),
      );
      request.headers['Authorization'] = 'Bearer $token';
      request.files
...
""";
  ...
  Widget build(BuildContext context) {
    ...
    final imageUrl = (venue['primary_image'] != null &&
        ...
        ? "http://192.168.1.10:8000/storage/${venue['primary_image']['image_url']}"
        : '';
