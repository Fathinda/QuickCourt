...
class _RegisterVenueScreenState extends State<RegisterVenueScreen> {
  ...
  Future<void> _fetchFacilities() async {
    try {
      final response =
          await http.get(Uri.parse("http://192.168.1.19:8000/api/facilities"));
  ...
  Future<void> _fetchCities() async {
    final response = await http.get(
      Uri.parse("http://192.168.1.19:8000/api/city"),
    );
    if (response.statusCode == 200) {
      setState(() {
  ...
  Future<void> _submitVenue() async {
    ...
    var request = http.MultipartRequest(
      "POST",
      Uri.parse("http://192.168.1.19:8000/api/owner/venues"),
    );
